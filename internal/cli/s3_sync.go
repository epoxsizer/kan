...
type syncEngine struct {
	...
	mutations        syncMutationRepository
	config           config.Sync
	client           s3SyncClient
	statePath        string
...
type syncMutationRepository interface {
	Backup(context.Context, string) error
	ImportDocument(context.Context, domain.ExportDocument, bool) error
}

func newSyncEngine(repo *storage.Repository, mutations syncMutationRepository, database string, syncConfig config.Sync, workingDirectory string, logger *slog.Logger, client s3SyncClient) (*syncEngine, error) {
...
func (engine *syncEngine) reconcile(ctx context.Context) error {
...
func (engine *syncEngine) importRemote(ctx context.Context, document domain.ExportDocument, dataHash, etag string, backup bool) error {
	if backup {
		...
		if err := engine.mutations.Backup(ctx, path); err != nil {
	...
	if err := engine.mutations.ImportDocument(ctx, document, true); err != nil {
