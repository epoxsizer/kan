...
func TestBackupRotationRemovesOnlyExpiredGeneratedBackups(t *testing.T) {
	...
	for _, name := range []string{"kan-auto-20260616-120000.db", "release-20260616-120000.db", "kan-pre-sync-20260616-120000.db"} {
	...
	removed, err := rotateBackups(directory, config.Backup{Retention: "336h"}, now)
...
func TestBackupRotationCanBeDisabled(t *testing.T) {
	directory := t.TempDir()
	path := filepath.Join(directory, "kan-auto-20260101-000000.db")
	require.NoError(t, os.WriteFile(path, []byte("backup"), 0o600))

	removed, err := rotateBackups(directory, config.Backup{Retention: "0"}, time.Now())
	require.NoError(t, err)
