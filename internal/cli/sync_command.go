	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return withSyncEngine(cmd, opts, func(engine *syncEngine) error {
				...
				fmt.Fprintln(cmd.OutOrStdout(), "sync pull complete")
				return nil
			})
		},
	}
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return withSyncEngine(cmd, opts, func(engine *syncEngine) error {
				...
				fmt.Fprintln(cmd.OutOrStdout(), "sync push complete")
				return nil
			})
		},
	}
...
func withSyncEngine(cmd *cobra.Command, opts *options, action func(*syncEngine) error) error {
	...
	engine, err := newSyncEngine(res.repo, res.config.Database, res.config.Sync, workingDirectory, res.logger, realS3SyncClient{})
	if err != nil {
		return err
	}
