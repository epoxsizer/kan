...
func openTestSyncEngine(t *testing.T, client s3SyncClient) *syncEngine {
	...
	engine, err := newSyncEngine(repo, repo, database, testSyncConfig(), directory, slog.New(slog.NewTextHandler(io.Discard, nil)), client)
	require.NoError(t, err)
...
func TestReconcileImportsRemoteOnlyChange(t *testing.T) {
	remote := &fakeSyncClient{}
	engine := openTestSyncEngine(t, remote)
	require.NoError(t, seed.Demo(context.Background(), engine.repo))
	...
	source := openTestSyncEngine(t, &fakeSyncClient{})
	remoteProject := domain.Project{Name: "Remote change", Position: 1024}
	require.NoError(t, source.repo.CreateProject(context.Background(), &remoteProject))
	remoteSnapshot, err := source.snapshot(context.Background())
	require.NoError(t, err)
	remote.body = remoteSnapshot.JSON
	remote.etag = `"remote-2"`

	require.NoError(t, engine.reconcile(context.Background()))
	projects, err := engine.repo.ListProjects(context.Background())
	require.NoError(t, err)
	require.Len(t, projects, 1)
	require.Equal(t, "Remote change", projects[0].Name)
}
